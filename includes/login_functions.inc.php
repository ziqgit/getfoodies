...
function check_login($dbc, $email = '', $pass1 = '') {
    ...
    if (mysqli_num_rows($ip_check_result) > 0) {
        ...
        if ($ip_lockout_until > time()) {
            ...
            error_log("check_login: IP address " . $ip_address . " is locked. Lockout until: " . $ip_row['lockout_until']);
		...
		if (mysqli_num_rows($result) == 1) {
            ...
            if (strlen($stored_hash) === 40 && password_verify($p, $stored_hash)) {
                ...
                if ($new_attempts_user < $max_failed_attempts_user) {
                    ...
                    error_log("check_login: User ID " . $row['user_id'] . " has " . $attempts_remaining . " attempts remaining.");
                }
    ...
    error_log("check_login: Returning success status: false");
